...
# 1. Introduction and background {#coursework .unnumbered}
...
model for a fluid, it maps onto the Ising model. We say that they are *isomorphic* to one another. This isomorphism extends the
...
the average number density of particles on our lattice $\rho=N/L^d$ jumps discontinuously from a
low to a high value.
...
* *The full width at half maximum (FWHM) of the structure factor can be used to estimate the correlation length $\xi$ through $\xi=1/FWHM$. Plot the temperature dependence of $\xi$ and comment on its behaviour*. **[4 marks]**
