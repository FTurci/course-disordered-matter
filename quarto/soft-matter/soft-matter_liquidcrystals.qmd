...
To understand where these terms come from microscopically, we can express the scalar order parameter in terms of the **probability distribution** $p(\Omega)$ of finding a rod with polar angles $\Omega=(\theta,\phi)$ in a coordinate system with its polar axis along the director $\mathbf{n}$. The infinitesimal probability is $p(\Omega)d\Omega$.
...
$$\mathcal{S}=\dfrac{3}{2}\left\langle(\mathbf{u} \cdot \mathbf{n})^2-\frac{1}{3}\right\rangle = \dfrac{1}{2}\langle 3 \cos^2\theta-1\rangle=\langle P_2(\cos\theta)\rangle = 2\pi \int_0^{\pi}P_2(\cos\theta)p(\theta)\sin\theta d\theta$$


A complementary approach is to the macroscopic free energy picture of Landau-de Gennes is to  consider the minimal physical ingredients that lead to the transition itself. As in the case of the colloids, we have essentially two classes
