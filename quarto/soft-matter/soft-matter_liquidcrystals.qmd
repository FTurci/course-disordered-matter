...
where we enforced that the trace is zero. The componets are given by
...
The eigenvector is found by solving the linear system
$$
(\mathbf{Q}-\mathcal{S} \mathbf{I}) \mathbf{n} = 0.
$$

It yields 

$$\mathbf{n} = \begin{pmatrix} \cos \psi \\ \sin \psi \end{pmatrix}$$

where the angle $\psi$ is given by
$$\psi = \arctan\left(\frac{Q_{xy}}{Q_{xx}}\right)$$

Since it is an arctangent, its periodicity is $\pi$ and not $2\pi$, which is consistent with the head-tail symmetry of the nematic phase.

This means that there is an angle $\alpha = 2\psi$ which is unique and represents the orientation of the director in the nematic phase.
