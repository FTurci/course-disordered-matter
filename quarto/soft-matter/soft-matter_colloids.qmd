...
def double_layer(h, R,B,ns ):
    lambdaB = 0.0007 # fixed Bjerrum length (micron)
...
```{pyodide}
#| caption: "DLVO potential [press ctrl/command [ENTER] to run]"
#| autorun: true
# parameters: CHANGE them to explore the interaction!
...
plotting.multi_plot(h, W_vdW, W_DR,DLVO,
    xlabel="h", ylabel="Interaction strength")
# nicer cliping
...
3.  Large concentration of salt depress the DVLO maximum altogether and eventually the Van der Waals interaction dominates.
:::
...
These fundamentally repulsive interaction mean that that, in a system with $N$ colloids, an additional $N+1$ colloid does not have acces to the entire configuration space: there is a large **excluded volume** due to the presence of the other colloids. These *excluded volume interactions* clearly have an **entropic effect:** they modify the number of accessible configurations.
