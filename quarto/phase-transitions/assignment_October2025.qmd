...
* Familiarise yourself with the main features of the program. Pay attention to how to change the temperature, number of lattice sites $N=L^2$, and the number of equilibration and sampling sweeps. 
...
In this implementation, the particle density is fixed at $\rho = \rho_c = 0.5$. This choice ensures that the system approaches the critical point as the temperature is lowered within the supercritical regime, $T_c \le T \lesssim 2T_c$. For convenience, the program uses dimensionless units by setting $\epsilon = k_B = 1$. The linear system size is intially set to $L=50$.
