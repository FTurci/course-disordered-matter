...
if __name__ == "__main__":
    ...
    n_meas = 2000
    T_values = np.linspace(0.55, 1.0, 6)
    ...
    for T in T_values:
        ...
        plt.xlabel("r")
        plt.ylabel("g(r)")
        plt.title(f"Average radial distribution function: T={T:.2f}, L={L}")
        plt.savefig(root_dir + f"figures/{prefix}_radial_distribution.pdf")
        np.savetxt(root_dir + f"data/{prefix}_radial_distrobution.csv",
                   np.column_stack((r_vals, g_avg)), delimiter=",", header="r,g(r)", comments='')

        plt.figure()
        plt.imshow(S_avg, origin="lower")
        plt.title(f"2D Structure Factor S(k): T={T:.2f}, L={L}")
        plt.colorbar()
        ...
        plt.xlabel("k")
        plt.ylabel("S(k)")
        plt.title(f"Radial Average of Structure Factor: T={T:.2f}, L={L}")
        plt.savefig(root_dir + f"figures/{prefix}_structure_factor_radial.pdf")
        np.savetxt(root_dir + f"data/{prefix}_structure_factor_radial.csv",
                   np.column_stack((r_S, S_rad)), delimiter=",", header="k,S(k)", comments='')

        plt.figure()
        plt.imshow(lattice, cmap="gray")
        plt.axis("off")
        plt.title(f"Final Lattice Configuration: T={T:.2f}, L={L}")
        plt.savefig(root_dir + f"figures/{prefix}_final_lattice.pdf")

    # ---- Aggregate results across T ----
    T_arr = np.array(T_values)
    C_arr = np.array(C_per_site_list)
    E_arr = np.array(E_per_site_list)

    prefix_global = f"L{L}_rho{density:.2f}"

    np.savetxt(root_dir + f"data/{prefix_global}_heat_capacity_vs_T.csv",
               np.column_stack((T_arr, C_arr)), delimiter=",",
               header="T,C_per_site", comments='')
    np.savetxt(root_dir + f"data/{prefix_global}_energy_vs_T.csv",
               np.column_stack((T_arr, E_arr)), delimiter=",",
               header="T,E_per_site", comments='')
    ...
    plt.savefig(root_dir + f"figures/{prefix_global}_heat_capacity_vs_T.pdf")
    ...
    plt.grid(True, alpha=0.3)
    plt.savefig(root_dir + f"figures/{prefix_global}_energy_vs_T.pdf")
