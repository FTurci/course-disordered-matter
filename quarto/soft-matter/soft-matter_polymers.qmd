...
<svg width="400" height="80" viewBox="0 0 400 80" xmlns="http://www.w3.org/2000/svg">
    ...
    <!-- Main backbone: a gently undulating line -->
    <polyline
        fill="none"
        stroke="#2b6cb0"
        stroke-width="4"
        ...
        "
    />
    <!-- Small random oscillations: overlay with thinner, lighter line -->
    <polyline
        fill="none"
        stroke="#90cdf4"
        stroke-width="2"
    ...
    />
    <!-- Monomer dots -->
    <circle cx="10" cy="40" r="4" fill="#2b6cb0"/>
    <circle cx="90" cy="45" r="4" fill="#2b6cb0"/>
    <circle cx="170" cy="42" r="4" fill="#2b6cb0"/>
    <circle cx="250" cy="43" r="4" fill="#2b6cb0"/>
    <circle cx="330" cy="41" r="4" fill="#2b6cb0"/>
    <circle cx="370" cy="38" r="4" fill="#2b6cb0"/>
    <!-- Optional: label -->
    <text x="200" y="75" text-anchor="middle" font-size="16" fill="#2b6cb0" font-family="sans-serif">
        Nearly straight polymer chain (random walk)
    </text>
</svg>
...
Again, we get $\left\langle R^{2}\right\rangle \propto N$, which suggests that a long freely-rotating chain can be represented by an *effective* freely-jointed chain with $N$ 'segments of length $b$. Real and effective chain must have the same actual length 

$$N b_{0}=N^{\prime} b$$

and the same end-to-end distance $<R^{2}>$ i.e. $$C N b_{0}^{2}=C N^{\prime} b^{2}.$$ 
