...
for some scalar value $\mathcal{S}$ (the order parameter between 0 for the isotropic and 1 for the nematic). Adopting this standard form allows us to construct the Landau-de Gennes free energy functional using its classic, canonical coefficients.
...
We also note that the expression for the nematic order parameter satisfies head-tail symmetry by employing the first non-trivial [Legendre polynomial](https://mathworld.wolfram.com/LegendrePolynomial.html){target="_blank"}, $P_2(\cos\theta)$ as 
