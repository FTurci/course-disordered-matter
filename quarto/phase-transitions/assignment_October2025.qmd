---
title: "Preparatory reading and setup for coursework assignment: Investigating correlations and phase behaviour in the 2d lattice-gas model"
format: html
---
...
vacant. The complete list of these occupancies $\{c\}$ specifies a microstate. The average particle number density (fraction of occupied
sites) is given by
...
In the **grand canonical ensemble**, the number of particles $N$ is allowed to fluctuate, and configurations are sampled with a probability proportional to the Boltzmann weight:

$$
P(C) \propto e^{-\beta [E(C) - \mu N(C)]},
$$

where $E(C)$ is the energy of configuration $C$, $N(C)$ is the number of particles in that configuration, $\mu$ is the chemical potential, and $\beta = 1/kT$. To generate such configurations efficiently, one can use the **Metropolis algorithm** adapted for grand canonical sampling:
...
### Fixed-Particle-Number Case (Canonical Ensemble)
...
Other observables (those which are second derivatives of the free energy) such as the specific heat capacity can't be calculated as simple averages. Instead they are calcululated as a variance. The formula for the specific heat (see problem sheets) is 
...
* On the PHYSM0071 Blackboard page open the Resources and Tools tab 
...
**The assignment itself will be released on Monday 13th October on Blackboard via the Assessment, Submission and Feedback tab. The submission deadline is Monday 27th October at 09:30.**
...
## 3.2 Isomorphism of the lattice gas and Ising model

*Show that the Hamiltonian of the Lattice Gas model in the grand canonical ensemble*
...
- *Add a function to calculate and print the dimensionless total energy and specific heat of the system.* 
  **[5 marks]**



## 3.3 Computational investigations of correlations on the approach to criticality

- *Perform simulations at various temperatures within the range $T_c \le T \le 1.0$, keeping the particle density fixed at $\rho = 0.5$. For each temperature, save the final configuration and describe how the overall structural characteristics evolve with temperature.* **[4 marks]**
...
* *The full width at half maximum (FWHM) of the structure factor can be used to estimate the correlation length $\xi$ through $\xi=1/FWHM$. Plot the temperature dependence of $\xi$ and comment on its variation with $T$*. **[4 marks]**


## 3.4 Temperature and system size dependence of the specific heat
