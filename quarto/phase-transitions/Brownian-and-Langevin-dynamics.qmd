...
But if we assume that there is some correlation time $t_c$ over which
\[
\langle f(t_1) f(t_2) \rangle = g(t_1 - t_2)
\]
decays rapidly then as long as we consider timescales $ \gg t_c $ we can safely replace $g(t_1 - t_2)$ by a delta function. Thus  we can approximate the noise by
...
Thus:
- At short times ($\gamma t \ll 1$): $\langle v \rangle \approx v_0$ — friction is negligible.
- At long times ($\gamma t \gg 1$): $\langle v \rangle \to 0$ — the system loses memory of the initial velocity.
...
So:
- Short time: $\langle v^2 \rangle \approx v_0^2$
- Long time: $\langle v^2 \rangle \to \Gamma / (2\gamma)$
