...
![An $N$-step domain wall in an Ising lattice.](Figs/domainwall.001.png){width=40% #fig-domainwall}
...
**(a)** The equilibrium state corresponds to the minimum of the free energy $\partial F/\partial M=0$.
...
**(b)** The near critical power law scaling of the magnetisation is $m\propto t^\beta$ and $m\propto H^{1/\delta}$.  To find a
...
**(c)** The choice of scale $a_4=1/4$ and $\tilde{a_2}=1/2$ yields the given form of the scaling function.
...
## 12.  Einstein’s expression for the diffusion coefficient
...
**(a)** Let $E_1 = E_2 = E_3$;  then $p_1 = p_2 = p_3$ in equilibrium.
...
$$
\dot{p}_i = \sum_j v_{ij} (p_j - p_i)
$$

i.e. $\dot{p}_1 = -v_0 p_1 + v_0 p_2$ etc.

Or $\dot{p}_i \equiv M_{ij} p_j$,  
where

$$
M_{ij} = v_0 \begin{pmatrix}
-1 & 1 & 0 \\
1 & -2 & 1 \\
0 & 1 & -1
\end{pmatrix}
...
**(b)** Equilibrium $\dot{p} = 0 \Rightarrow M \cdot p = 0$  
Verify:
...
Hence $(1, 1, 1)/3$ is a steady-state solution.


**(c)** Uniqueness: $\dot{p} = 0$ requires $p$ to be an eigenvector with zero eigenvalue.  
The eigenvalues $v_0 \lambda$ obey:
...
$$
\Rightarrow \lambda(\lambda + 3)(\lambda + 1) = 0.
$$

So $\lambda = 0$ corresponds to only one eigenvector;  
the other two have $\lambda = -1$ and $\lambda = -3$.
