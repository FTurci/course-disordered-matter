...
The [IUPAC definition of colloids](https://goldbook.iupac.org/terms/view/C01172) is based on the idea that the particles are dispersed in a medium, creating a *subdivision* at the **colloidal scale**, which is approximately 1 nm to 1 µm.
...
[^soft-matter_colloids-1]: The citric acid in lemon juice lowers the pH of the water, making it more acidic. The pH is simply the negative logarithm of the concentration of hydrogen ions.
...
[^soft-matter_colloids-2]: This is an instance of the idea of **coarse graining** that we have introduced [earlier](soft-matter_intro.qmd).
...
1.  When the salt concentration is low, the double layer repulsion dominates the DLVO interaction.
...
3.  Large salt concentrations depress the DLVO maximum altogether and eventually the Van der Waals interaction dominates.
:::
...
4.  test if the new position is valid (no overlaps)
...
for step in range(steps):
    ...
    new_pos = pos.copy()
    new_pos[i] += move
    # sorting allows us to only check for consecutive particles
    new_pos.sort()
    if valid(new_pos):
        pos = new_pos
    if step % 100 == 0:
...
The new accessible volume is $$V_{\rm accessible}'=V-2V_{\rm exclusion}+V_{\mathrm{overlap}}$$. The interaction between the two colloids resulting from the free energy advantage is called **potential of mean force** $W(r)$. It is expressed as $$
\begin{aligned}
...
::: {style="border: 2px solid red; padding: 1em; border-radius: 8px; background-color: transparent;"}
$$
W_{\rm AO} (r) = - \dfrac{4 \pi \rho_p^r k_B T}{3} (R+\delta)^3\left[1-\dfrac{3}{4} \dfrac{r}{R+\delta}+\frac{1}{16}\left(\dfrac{r}{R+\delta}\right)^3\right] \quad 2R\leq r< 2R+2\delta
$$
:::
...
In the phase diagram below these different phases have been highlighted. It is clear that, compared to hard spheres which only display a fluid and a crystalline solid phase, systems like the Lennard-Jones fluid display an additional phase, the **liquid**. This phase emerges from the presence of the attractive well in the interaction potential. This allows for short-range attractive interactions that cause a region of the phase diagram in the same class of universality of the [**Ising model**](../phase-transitions/Ising-model.qmd) and the [**lattice gas**](../phase-transitions/lattice-gas.qmd).
...
# now try with hard spheres, using a custom library you will find in the course pages
...
for positions in trajectory:
    ...
    gs.append(_g)
plt.plot(r, np.mean(gs,axis=0), 'b-', label=rf"Hard spheres, $\phi={phi}$")
