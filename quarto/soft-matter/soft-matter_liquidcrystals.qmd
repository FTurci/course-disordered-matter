...
Given the free energy above, we want to find $p(\theta)$ that minimises it under the normalisation constraint $\int p(\theta) \sin \theta d \theta=1$. This is solve via Lagrange multipliers to obtain
...
::: column-margin
![Sir Charles Frank (1911-1998)](https://upload.wikimedia.org/wikipedia/en/a/ac/Frederick_Charles_Frank_Bristol.jpeg)
:::
...
Disclinations play a crucial role in the physics of liquid crystals, influencing their mechanical, optical, and dynamic properties. They can be visualized as points (in 2D) or lines (in 3D) where the orientational order breaks down.
