...
# The Ising model: the prototype model for a phase transition {#sec-Isingmodel}
...
$\langle\cdot\rangle$ means an average over typical configurations corresponding to the prescribed value of $J/k_BT$.
...
effects among a large number of spins. 

::: {#fig-snapshots layout-ncol=3}
![$T=1.2T_c$](Figs/supercrit.png){#fig-ssa}

![$T=T_c$](Figs/crit.png){#fig-ssb}

![$T=0.95T_c$](Figs/subcrit.png){#fig-ssc}
...
[An interactive Monte Carlo simulation of the Ising model](https://physics.weber.edu/schroeder/software/demos/isingmodel.html) demonstrates the phenomenology,  By altering the temperature you will be able to observe
for yourself how the typical spin arrangements change as one traverses the
...
In this representation, the partition function involves a count over all possible domain wall arrangements. Since the domain walls are non
interacting (eg it doesn't cost energy for one to move along the chain) they can be treated as independent.
Independent contributions to the partition function simply multiply. So we can calculate $Z$ by considering the partition function associated
...
an $m \times m$ matrix $\mathbf{V}$, known as the transfer matrix ($i,j,k$
...
@eq-Vs is simply just the trace of $\mathbf{V}^N$ (check this yourself for a short
...
V = Q T Q^\dagger,
$$
where $Q$ is a unitary matrix ($Q^\dagger Q = I$), and 
$T$ is upper triangular with the eigenvalues of $V$ on its diagonal:
$$
T = \begin{pmatrix}
\lambda_1 & * & \cdots & * \\
0 & \lambda_2 & \cdots & * \\
...
$|\mathbf{V}-\lambda\mathbf{I}|$. They are
