...
where $V_d$ is another dimension-dependent constant (eg. $4\pi/3$ for $d=3$). This expression (@fig-cnt-schem) exhibits a characteristic maximum at a critical droplet radius $R_c$, obtained by extremizing $\Delta F(R)$ with respect to $R$:
...
Now imagine that rather than positioning the system just inside the coexistence region we move immediately to a state point well inside the coexistence region (cf. @fig-spinodal) such that there is no metastable minimum in the free energy.  Then there is no nucleation and growth, rather the system is *unstable* and immediately starts to phase separate at all points in the system.  This so called *spinodal decomposition*. 
 ...
 ![Schematic phase diagram for a magnet showing the regions in which one observes nucleation and growth, or spinodal decomposition](Figs/spinodal.png){width=50% fig-align="center" #fig-spinodal}
...
dynamics, the order parameter can relax locally without constraint. Motion of interfaces is driven by local chemical potential differences. 
In the jargon of the field, this is called 'model A' dynamics.
...
corresponding to a dynamic exponent $z = 2$, where in general $L(t)\sim t^{1/z}$. 
The local chemical potential difference across a domain wall depends on its curvature. 
Thus regions of high curvature (small domains) shrink and are absorbed by larger, flatter ones.
...
This imposes a diffusive constraint on the dynamics (so-called 'model B' dynamics).
