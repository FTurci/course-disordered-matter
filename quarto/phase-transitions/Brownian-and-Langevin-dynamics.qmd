...
Hence the step size must shrink as the square root of the time step.
Only under this scaling does the random walk converge to a well-defined continuum process
with finite diffusion constant, namely Brownian motion or the Langevin equation.
...
![Sketch of $g(t_1−t_2)$ against $|t_1− t_2|$](Figs/autocorrelate.png){#fig-autocorrelate
width=60%} 
...
$$
where we have used the fact that $f(t)$ is random with  $\langle f(t)\rangle=0$.
Thus:
...
\textbf{Short times} ($\gamma t \ll 1$):
...
$$
which corresponds to \textit{diffusive motion} (distance travelled is proportional to $\sqrt{t}$) with diffusion constant
$D = \tfrac{k_B T}{\gamma}$.
