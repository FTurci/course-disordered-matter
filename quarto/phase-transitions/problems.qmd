...
$$
where $\Delta G_v < 0$ is the free energy change per unit volume due to the phase change, and $\sigma > 0$ is the interfacial energy between the solid and liquid phases.
...
An isolated system can occupy three possible states of the same energy. The kinetics are such that it can jump reversibly
 between states $1$ and $2$ and between states $2$ and $3$ but not directly between $1$ and $3$. Per unit time, there is a probability $\lambda_0$ that the 
