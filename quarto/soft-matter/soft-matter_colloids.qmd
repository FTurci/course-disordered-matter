...
$$
V_{\text {overlap }}(r)=\frac{\pi\left(4 (R+\delta)+r\right)\left(2 (R+\delta)-r\right)^2}{12}
$$

The new accessible volume is $$V_{\rm accessible}\prime=V-2V_{\rm exclusion}+V_{\mathrm{overlap}}$$. The interaction between the two colloids resulting from the free energy adgvantage is called **potential of mean force** $W(r)$. It is expressed as $$
\begin{aligned}
...
eta_p = 0.1       # Polymer packing fraction
...
def AO_potential(r, R, delta, rho_p, kBT):
    Rd = R + delta
...
plt.gca().set(xlabel="r", ylabel="g(r)", ylim= (0,2))
plt.legend()
plt.show()
...
$$ Since both $h(r)$ and $c(r)$ are unknown , an additional relationship is needed, called a **closure**. This is constructed through phsyical arguments. A common one is the so-called Percus-Yevick closure
...
It is easy to show (see below) that in 1 dimension the general solution is $$\rho(x, t) = \frac{1}{\sqrt{4\pi D t}} \exp\left(-\frac{x^2}{4Dt}\right)$$ 
