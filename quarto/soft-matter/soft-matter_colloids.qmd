...
$$\mu = k_BT \ln \eta_p^r$${#eq-ideal-mu}

where $\eta_p^r=\frac{\pi}{6} \rho_p^r \sigma_p^3 $ is therefore the **packing fraction** of the polymers in the ideal reservoir.
...
eta_p^r = 0.1       # Polymer concentration (dimensionless)
...
def AO_potential(r, R, delta, eta_p^r, kBT):
    Rd = R + delta
    # Only defined for 2R <= r < 2Rd
    W = np.zeros_like(r)
    mask = (r >= 2*R) & (r < 2*Rd)
    x = r[mask] / Rd
    W[mask] = - (4 * np.pi * eta_p^r * kBT / 3) * Rd**3 * (1 - 0.75 * x + 0.0625 * x**3)
    return W

W_AO = AO_potential(r, R, delta, eta_p^r, kBT)
