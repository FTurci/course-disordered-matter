...
Applies to a system of $N$ particles (or spins) in a fixed volume $V$ having adiabatic walls (so that the internal energy $E$ is constant (note sometimes one sees the symbol $U$ used instead of $E$). Denoted as constant-$NVE$.  Let $\Omega$ be the number of (micro)states having the prescribed energy: 

$$
\Omega=\sum_\textrm{all states having energy U}
$$
...
A useful quantity is the {\em partition function} 
...
The canonical ensemble is usually used to describe systems such as magnets, or a fluid held at constant volume. 
It is the ensemble we shall use most in this course. 
...
$$
and a state probability analogous to Eq.~(\ref{eq:Pi}) holds. One can recast this in a form similar to Eq.~\ref{eq:ZNVT}:
...
Again a state probability analogous to Eq.~(\ref{eq:Pi}) holds. 
...
Free energies are not directly observable quantities. However, all physical observables can be expressed in terms of {\em derivatives} of the free energy. 
