...
v_f=\frac{4 \pi}{3} \delta^3 
$ from which we can calculate the entropy
...
![Phase diagram of the Lennard-Jones Fluid](figs/LJPhaseDiagram.png)
...
| Buckingham (Exp-6)  | $U(r) = A e^{-Br} - \frac{C}{r^6}$                                               | Rare gases, molecular crystals                  |
| Morse               | $U(r) = D_e \left( e^{-2\alpha(r - r_e)} - 2e^{-\alpha(r - r_e)} \right)$         | Diatomic molecules, chemical bonds              |
| Square Well         | $U(r) = \begin{cases} \infty & r < \sigma \\ -\epsilon & \sigma \leq r < \lambda\sigma \\ 0 & r \geq \lambda\sigma \end{cases}$ | Model fluids, colloids with short-range attraction |
| Soft Sphere         | $U(r) = \epsilon \left( \frac{\sigma}{r} \right)^n$                              | Polymer colloids, soft repulsive systems        |
...
Experimentally, $g(r)$ can be measured using scattering techniques or direct imaging in colloidal systems. It provides insight into short-range order, local structure, and phase transitions in soft matter.
...
where we defined the total correlation function as $h(r)=g(r)-1$. The OZ equation is an integral equation. In Fourier space (assuming the isotropicity of a fluid) it becomes and algebraic equation
$$
...
def PercusYevickHS(phi,plot=True,filename=None):
    # number density
    rho=6./pi*phi
    ...
    if filename is not None:
        np.savetxt(filename, list(zip(r,g)))
    # plots
    if plot:
...
PercusYevickHS(0.4)
PercusYevickHS(0.5)
plt.title("Hard-spheres radial distribution fucntion from liquid state theory")
...
We have seen [earlier](../phase-transitions/Brownian-and-Langevin-dynamics.qmd) in the Unifying concepts section that if we are given a number of particles evolving microscopically according to the Langevin equation
...
where $\rho(\mathbf{r})$ is the proability of finding 
