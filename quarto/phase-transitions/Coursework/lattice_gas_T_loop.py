...
# Only include half the neighbors (e.g., right and down) to avoid double counting
HALF_NEIGHBORS = [(0, 1), (1, 0)]  # Right and down

@njit
def compute_total_energy(lattice, L):
    half_neighbors = ((0, 1), (1, 0))  # right and down only
    energy = 0.0
    for i in range(L):
        for j in range(L):
            n = lattice[i, j]
            for d in half_neighbors:
                ni = (i + d[0]) % L
                nj = (j + d[1]) % L
                energy -= n * lattice[ni, nj]
    return energy
...
if __name__ == "__main__":
 ...
 # Parse command-line arguments
    ...
    n_sweeps_per_meas = 20  # Sweeps between measurements
    n_meas = 2000  # Number of measuremento 

    for T in np.arange(0.35, 0.91, 0.05):
