...
The additional orientational degrees of freedom opens up the possibilities of phases that do not normally exist for spherical, isotropic particles.  We progress from the most disordered to the most ordered phase:
...
The rods are head-tail symmetric, so, we cannot truly distinguish which one is the head or the tail (e.g. an orientation of $\theta$ in 2D is the same as an orientation of $\theta+\pi$). We cannot therefore just take the average of the vectors to extract the common orientation of the various vectors, otherwise we would incur into a cancellation.

Instead, we construct a second moment tensor called the **alignment tensor**. There are different notations in the literature. The most general form is
...
The analysis of the tensor yield the main characteristics of the orientation of the system.  For example
...
To start constructing Q, we take the outer (tensorial) product of the unit vector with itself:


$$
\mathbf{u}_i \otimes \mathbf{u}_i = \begin{pmatrix} \cos^2 \theta_i & \cos \theta_i \sin \theta_i \\ \cos \theta_i \sin \theta_i & \sin^2 \theta_i \end{pmatrix}.
$$

The resulting matrix is clearly symmetric and has obviously trace equale to 1 because $\cos^2 \theta_i + \sin^2 \theta_i = 1$.  Angled brackets $\langle \cdot \rangle$ denote the average over all particles in the system. The alignment tensor is then defined as
$$
Q = 2\left\langle \mathbf{u}_i \otimes \mathbf{u}_i \right\rangle -\mathbf{I}
$$
where we used the identity matrix to zero out the trace. The factor 2 is a geometric normalisation, and we see in the following that it is convenient when looking at the magnitude of Q.

For convenience, let's write $\mathbf{Q}$ in terms of its components:

$$\mathbf{Q} = \begin{pmatrix} Q_{xx} & Q_{xy} \\ Q_{yx} & -Q_{xx} \end{pmatrix}$$

where we enforced that the trace is zero. The components are given by

$$Q_{xx} = \langle 2\cos^2 \theta_i - 1 \rangle = \langle \cos 2\theta_i \rangle$$

$$Q_{xy} = \langle 2\cos \theta_i \sin \theta_i \rangle = \langle \sin 2\theta_i \rangle$$

where we used the double angle trigonometric identities $\cos 2\theta = 2\cos^2 \theta - 1$ and $\sin 2\theta = 2\sin \theta \cos \theta$.


The magnitude of Q is denoted by $\mathcal{S}$ and defined as the largest eigenvalue of Q, and the corresponding eigenvector is the director. The eigenvalue problem is formally

$$
\mathbf{Q} \mathbf{n} = \mathcal{S} \mathbf{n} 
$$

By taking the determinant we have

$$
\operatorname{det}(\mathbf{Q}-\lambda \mathbf{I})=0,
$$
which gives the characteristic polynomial in terms of the components of Q:

$$-Q_{xx}^2-Q_{xy}^2+\lambda^2=0$$

So the eigenvalues are $\lambda = \pm \sqrt{Q_{xx}^2 + Q_{xy}^2}$, and the largest eigenvalue is the positive one, which we define as the nematic order parameter 

$$\mathcal{S} = \sqrt{Q_{xx}^2 + Q_{xy}^2}$$

The director $\mathbf{n}$ is the eigenvector of the alignment tensor $\mathbf{Q}$ corresponding to the largest eigenvalue $\mathcal{S}$.

Since $\mathbf{Q}$ is symmetric and traceless, it can be diagonalised using a rotation that aligns the director with the x-axis.


$$
\mathbf{Q}=\mathbf{R}(\psi)\left(\begin{array}{cc}
\mathcal{S} & 0 \\
0 & -\mathcal{S}
\end{array}\right) \mathbf{R}(\psi)^T
$$

where 

$$
\mathbf{R}(\psi)=\left(\begin{array}{cc}
\cos \psi & -\sin \psi \\
\sin \psi & \cos \psi
\end{array}\right) .
$$

is a rotation of angle $\psi$ that aligns the director with the x-axis. The angle $\psi$ is the orientation of the director!

Carrying out the matrix multiplication explicitly yields the components in terms of the director:

$$
\begin{aligned}
&Q_{x x}=\mathcal{S}\left(\cos ^2 \psi-\sin ^2 \psi\right)=\mathcal{S} \cos (2 \psi)\\
&Q_{x y}=\mathcal{S}(2 \sin \psi \cos \psi)=\mathcal{S} \sin (2 \psi)
\end{aligned}
$$

Taking the ratio of the two components gives the angle of the director:
$$\tan(2\psi) = \frac{Q_{xy}}{Q_{xx}}$$

Notice that the double angle comes out naturally, and reflects the head tail symmetry.


This lends itself to a complex plane representation of the director and the order parameter:

- The director is simply $\mathbf{n} = e^{i\psi}$
- the order parameter Q can be mapped to a complex number $Q = Q_{xx} + i Q_{xy} = \mathcal{S} e^{i2\psi}$ where the 2 in the exponent reflects the head-tail symmetry of the nematic phase.
...
For historical reasons, in three dimensions, the alignment tensor is often defined as

$$
\mathbf{Q}=\mathcal{S}\left(\mathbf{n} \otimes \mathbf{n}-\frac{1}{3} \mathbf{I}\right)
$$

for some scalar value $\mathcal{S}$ (the order parameter between 0 for the isotropic and 1 for the nematic). Notice that this is slighlty different from the general definition above, but it is equivalent up to a multiplicative factor. We will work with this definition because it consistently produces the standard Landau-de Gennes free energy functional without further manipulations.

If we choose now the $z$ axis along $\mathbf{n}$ we can write this explicitly as 
...
$$
f=f_0+\frac{A}{3} S^2-\frac{2 B}{27} S^3+\frac{C}{9} S^4
$$
...
We can also construct a more microscopically motivated theory. We can start by linking the scalar order parameter in terms of the **probability distribution** $p(\Omega)$ of finding a rod with polar angles $\Omega=(\theta,\phi)$ in a coordinate system with its polar axis along the director $\mathbf{n}$. The infinitesimal probability is $p(\Omega)d\Omega$.

The nematic phase has full **cylindrical symmetry** around the director, meaning $p(\theta, \phi)$ is independent of the azimuthal angle $\phi$ and depends only on the deviation $\theta$. Accounting for head-tail symmetry via the first non-trivial [Legendre polynomial](https://mathworld.wolfram.com/LegendrePolynomial.html){target="_blank"}, $P_2(\cos\theta)$, the scalar order parameter is defined as:

$$\mathcal{S}=\dfrac{3}{2}\left\langle(\mathbf{u} \cdot \mathbf{n})^2-\frac{1}{3}\right\rangle = \dfrac{1}{2}\langle 3 \cos^2\theta-1\rangle=\langle P_2(\cos\theta)\rangle = \int_0^{\pi}P_2(\cos\theta)p(\theta)\sin\theta d\theta$$


A complementary approach to the macroscopic free energy picture of Landau-de Gennes is to  consider the minimal physical ingredients that lead to the transition itself. As in the case of the colloids, we have essentially two classes
...
- a generic entropic contribution of the form $k_B\int p(\Omega)\ln p(\Omega) d\Omega$ using the [Gibbs entropy formula](https://en.wikipedia.org/wiki/Entropy_(statistical_thermodynamics)#Gibbs_entropy_formula){target="_blank"}.
...
The transition is **weakly-first** order, as two separate basins of stability (separated typically by a small barrier) are formed.
...
These profiles are example of simple **free energy landscapes**. We will see more about this later in  @sec-energy-landscapes.
...
fig, axs = plt.subplots(1, 3, figsize=(9.5, 3.5))
steps = [1/6, 0.5, 1.0]
titles = [
...
When two disclinations of opposite charges meet, they **annihilate**. This means the defects can cancel each other out, restoring uniform orientational order in the region. The annihilation of defect pairs is a key mechanism by which nematic liquid crystals relax toward equilibrium after being disturbed. This is because the idealise equilibrium state should in principle be free of any such defects, as they cost energy: they deform the director field on long distances, leading to a deformation cost, and they also engenders a complete loss of orientational order at the singularity point, which costs energy by itself.
